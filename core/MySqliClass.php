...
class MySqliClass{
    ...
    public function connectServer(){
        global $config;
        $this->audit = !empty($config['auditable'])? $config['auditable'] : false;
        $this->audit_table = !empty($config['audit_table'])? $config['audit_table'] : null;
        try {
            if (empty($database)) {
                $this->con = new mysqli(
                    $config['accessBD']['DB_HOST'].":".$config['accessBD']['DB_PORT'],
                    $config['accessBD']['DB_USER'], 
                    $config['accessBD']['DB_PASS']);
            }else {
                $this->con = new mysqli(
                        $config['accessBD']['DB_HOST'].":".$config['accessBD']['DB_PORT'],
                        $config['accessBD_'.$database]['DB_USER'], 
                        $config['accessBD_'.$database]['DB_PASS']);
            }
            
            if ($this->con->connect_errno) {
                error_log(print_r("Fallo de la conexión a MySQL: (" . $this->con->connect_error . ") ", true));
            }
        }  catch (Exception $e) {
            error_log(print_r("Exception MySQL: (" . $e->getMessage() . ") ", true));
        }
        return $this->con;
    }
        ...
        try {
            if (empty($database)) {
                ...
                $this->con = new mysqli(
                        ...
                        $config['accessBD_'.$database]['DB_USER'], 
                        $config['accessBD_'.$database]['DB_PASS'],
                        $config['accessBD_'.$database]['DB_NAME']);
            }
